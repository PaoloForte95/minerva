...
public class MaterialFlowOptimizationProblem extends AbstractOptimizationProblem{
	...
	public double evaluateMachineProductivity (int robotID ,int taskID,int pathID,double pathDelay) {
		...
		if(IDsIdleRobots.contains(robotID) && realTasksIDs.contains(taskID)) {
			...
			double bucket_capacity = bucketCapacities.get(robotID);
			...
			machineProductivity = (fill_factor*bucket_capacity/real_time)/(fill_factor*bucket_capacity/nominal_time);
