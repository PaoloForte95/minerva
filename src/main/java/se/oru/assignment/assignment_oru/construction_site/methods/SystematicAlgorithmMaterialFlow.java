package se.oru.assignment.assignment_oru.construction_site.methods;
...
public class SystematicAlgorithmMaterialFlow extends AbstractOptimizationAlgorithm{
					...
					for(int s = 0; s < maxNumPaths; s++) {
						if ( AssignmentMatrix[i][j][s] > 0) {
							...
							if (linearWeight != 1) {
								...
								fleet_productivity = fleet_productivity + oap.evaluateMachineProductivity(robotID,taskID,s,costF)/IDsAllRobots.size();
								System.out.println("TEST PRODUCTIVITY : " + fleet_productivity);
								...
								double costB = optimizationModel.objective().getCoefficient(optimizationModel.variables()[i*numTaskAug*maxNumPaths+j*maxNumPaths+s]);
								fleet_productivity = oap.evaluateMachineProductivity(robotID,taskID,s,0);
								//System.out.println("TEST PRODUCTIVITY COST: " + productivityCost);
	 ...
	 * @return An Optimal Assignment that minimize the objective function
	 */	
	public double [][][] solveOptimizationProblem2(MPSolver optimizationProblem,MaterialFlowOptimizationProblem oap){
